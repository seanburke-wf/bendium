...
Future flashBadge(String text, {Duration duration: const Duration(seconds: 3)}) {
  chrome.browserAction.setBadgeText(new chrome.BrowserActionSetBadgeTextParams()..text = text);
  return new Future.delayed(duration, clearBadge);
}

/// Set the text of the icon back to blank.
void clearBadge() {
  chrome.browserAction.setBadgeText(new chrome.BrowserActionSetBadgeTextParams()..text = '');
}